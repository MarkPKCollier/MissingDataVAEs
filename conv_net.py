...
parser.add_argument('--dataset', type=str, default='mnist', help='binary_minst | mnist | cifar10')
...
def decoder(z, b=None):
    ...
    for i, layer in enumerate(p_x_b_z_layers):
        ...
        if i == (len(p_x_b_z_layers) - 1):
            p = tf.layers.conv2d_transpose(net, filters, kernel_size,
                                           strides=strides, activation=activation,
                                           padding='SAME')

            bias_p = tf.get_variable('p_x_b_z_layer_{0}_bias_p'.format(i),
                                   shape=p_x_b_z_layers[i][1],
                                   initializer=tf.initializers.zeros())

            p = tf.nn.bias_add(p, bias_p)

            if args.predict_variance:
                log_var = tf.layers.conv2d_transpose(net, filters, kernel_size,
                                                     strides=strides, activation=activation,
                                                     padding='SAME')

                bias_log_var = tf.get_variable('p_x_b_z_layer_{0}_bias_log_var'.format(i),
                                   shape=p_x_b_z_layers[i][1],
                                   initializer=tf.initializers.zeros())

                log_var = tf.nn.bias_add(log_var, bias_log_var)
            else:
                log_var = None
        else:
            net = tf.layers.conv2d_transpose(net, filters, kernel_size,
...
def model(features, labels, mode, params):
    ...
    def log_probs(z_sample):
        ...
        if args.dataset == 'binary_mnist':
        ...
        elif args.predict_variance and (args.dataset == 'cifar10' or args.dataset == 'mnist'):
            x_pred = x_logits
            ...
            # log_prob_full = -0.5 * (tf.cast(tf.log(2 * np.pi), tf.float32) + x_log_var + tf.divide(tf.square(x - x_pred), x_var))
            log_prob = tf.reduce_mean(
        ...
        elif args.learn_variance and (args.dataset == 'cifar10' or args.dataset == 'mnist'):
            x_pred = x_logits
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        return tf.estimator.EstimatorSpec(mode,
            predictions={'x': tf.clip_by_value(x_pred, 0.0, 1.0), 'z': mu},
            export_outputs={'y': tf.estimator.export.ClassificationOutput(
                scores=tf.clip_by_value(x_pred, 0.0, 1.0))})
...
for missingness_type, p in [('independent', 0.5)]:
    ...
    # for model_type, title in [('VAE_ind_decoder_b', 'Zero imputation w/ encoder + decoder indicator variables')]:
        ...
        if args.save_imgs and 'mnist' in args.dataset:
            ...
            plt.savefig('mnist_{0}.png'.format(model_type), bbox_inches='tight')
            plt.close()
        elif args.save_imgs and args.dataset == 'cifar10':
            def draw_digit(data, row, col, n):
                size = 32
                plt.subplot(row, col, n)
                plt.imshow(data)
            ...
            for i in range(show_size):
                for j in range(show_size):
                    ...
                    total += 1
            fig.suptitle(title, fontsize=35, verticalalignment='center', y=0.92)
            plt.savefig('cifar10_{0}.png'.format(model_type), bbox_inches='tight')
            plt.close()
        ...
        image_plot_input_fn = tf.estimator.inputs.numpy_input_fn(
            {'x': x_test[:30], 'x_input': x_test_input[:30], 'b': b_test[:30],
            ...
            if args.save_imgs and 'mnist' in args.dataset:
                ...
                for i in range(show_size):
                    for j in range(show_size):
                        ...
                        total += 1
                fig.suptitle(title, fontsize=35, verticalalignment='center', y=0.92)
                plt.savefig('recon_cifar10_{0}.png'.format(model_type), bbox_inches='tight')
                plt.close()
